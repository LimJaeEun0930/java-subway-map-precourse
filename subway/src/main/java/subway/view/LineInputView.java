...
import static main.java.subway.appconfig.ErrorConstants.ERROR_ALREADY_EXIST_LINE;
import static main.java.subway.appconfig.ErrorConstants.ERROR_LINE_NAME_NOT_END_WITH_SUFFIX;
import static main.java.subway.appconfig.ErrorConstants.ERROR_NON_EXIST_LINE;
import static main.java.subway.appconfig.ErrorConstants.ERROR_TOO_SHORT_LINE_NAME;
...
public class LineInputView extends InputView {
    ...
    public Station getNewLineStartStation() {
        while (true) {
            ...
            if (isInStations(input)) {
                return StationRepository.stations().stream().filter(station -> station.getName().equals(input)).findFirst().get();
            }
        }
    }
    ...
    private static boolean isInStations(String input) {
        return StationRepository.stations().stream().filter(station -> station.getName().equals(input)).findFirst().isPresent();
    }
}
